...
export class OfflineRouter {
  ...
  async getReachableNetworkGeoJSON(anchorCoord, { mode } = {}) {
    ...
    if (!this.pathFinder || typeof this.pathFinder.buildReachableSegments !== 'function') {
      return null;
    }

    const snap = this.pathFinder.findNearestPoint(anchorCoord);
    if (!snap) {
      return null;
    }
    ...
    if (snap.type === 'node') {
      ...
      pushKey(snap.edgeStart);
      pushKey(snap.edgeEnd);
    }
    ...
    const segments = this.pathFinder.buildReachableSegments(Array.from(seedKeys), mode);
    ...
    const features = segments
      .map((segment) => {
        const start = sanitizeCoordinate(segment?.start);
        const end = sanitizeCoordinate(segment?.end);
        if (!start || !end) {
          return null;
        }
        return {
